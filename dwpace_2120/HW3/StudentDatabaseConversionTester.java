...
public class StudentDatabaseConversionTester {
	...
	@Test
	public void testSortStudentDatabase() {
		try {
			String[] arguments = { "studentJUnitTestData.csv", "sortOutput.ser" };
			ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
			PrintStream printStream = new PrintStream(byteStream);
			System.setOut(printStream);// java api should overload this method and just take byteStream
			SortStudentDatabase.main(arguments);
			assertEquals("Database organized", byteStream.toString());
			arguments[0] = "sortOutput.ser";
			arguments[1] = "sortOutput.csv";
			byteStream.reset();
			SortStudentDatabase.main(arguments);
			assertEquals("Database organized", byteStream.toString());
			arguments[0] = "sortOutput.csv";
			arguments[1] = "sortOutput2.csv";
			byteStream.reset();
			SortStudentDatabase.main(arguments);
			assertEquals("Database organized", byteStream.toString());
			arguments[0] = "sortOutput.ser";
			arguments[1] = "sortOutput3.ser";
			byteStream.reset();
			SortStudentDatabase.main(arguments);
			assertEquals("Database organized", byteStream.toString());
		} catch (Exception e) {
	...
	/*
	 * @Test public void testFormattedToSerialized() throws Exception{ String infile
	 * = "studentJUnitTestData.csv"; String outfile =
	 * "studentJUnitTestData_out5.ser";
	 * StudentDatabaseIO.formattedToSerialized(infile,outfile);
	 * 
	 * //OK, so at this point, the serialized file should contain a formatted
	 * version //of the data we read in from studentJUnitTestData.csv
	 * 
	 ...
	 * ; StudentDatabase db2 = StudentDatabaseSerializedFileReader.readData();
	 * StudentDatabaseSerializedFileReader.closeFile(); // now compare the two
	 * databases assertEquals(db.getNumStudents(), db2.getNumStudents()); for (int
	 * i=0; i<db.getNumStudents(); i++) { Student s1 = db.getStudent(i); Student s2
	 * = db2.getStudent(i); assertEquals(s1.getStudentID(),s2.getStudentID());
	 * assertEquals(s1.getSex(),s2.getSex());
	 * assertEquals(s1.getEthnicGroup(),s2.getEthnicGroup());
	 * assertEquals(s1.getFirstName(),s2.getFirstName());
	 * assertEquals(s1.getLastName(),s2.getLastName());
	 ...
	 * }
	 * 
	 * @Test public void testSerializedToFormatted() throws Exception{
	 * StudentDatabaseCSVFileReader.openFile("studentJUnitTestData.csv");
	 * StudentDatabase db = StudentDatabaseCSVFileReader.readData();
	 * StudentDatabaseCSVFileReader.closeFile();
	 * 
	 ...
	 * StudentDatabaseSerializedFileWriter.closeFile();
	 * 
	 * String infile = "studentJUnitTestData_out6.ser"; String outfile =
	 * "studentJUnitTestData7.csv";
	 * StudentDatabaseIO.serializedToFormatted(infile,outfile);
	 * 
	 * // now studentJUnitTestData7.csv should be a sorted version of
	 * studentJUnitTestData.csv
	 * 
	 * db.sortByGPA();
	 * 
	 ...
	 * StudentDatabase db2 = StudentDatabaseCSVFileReader.readData();
	 * StudentDatabaseCSVFileReader.closeFile(); // check that they are the same
	 * assertEquals(db.getNumStudents(), db2.getNumStudents()); for (int i=0;
	 * i<db.getNumStudents(); i++) { Student s1 = db.getStudent(i); Student s2 =
	 * db2.getStudent(i); assertEquals(s1.getStudentID(),s2.getStudentID());
	 * assertEquals(s1.getSex(),s2.getSex());
	 * assertEquals(s1.getEthnicGroup(),s2.getEthnicGroup());
	 * assertEquals(s1.getFirstName(),s2.getFirstName());
	 ...
	 * assertEquals(s1.getFAFSA(),s2.getFAFSA());
	 * assertEquals(s1.getTakePrgrs(),s2.getTakePrgrs(),0.01);
	 * assertEquals(s1.getFinancialNeed(),s2.getFinancialNeed(),0.01); } } // end
	 * test
	 */
